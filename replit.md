...
- **PWA Version Guard (Julho/2026):** Rede de segurança independente do service worker para atualização automática pós-deploy. Cada build gera um `BUILD_VERSION` único (vite `define` → `__APP_BUILD_VERSION__` no bundle + `dist/version.json` emitido no build). O backend expõe `GET /api/version` (público, sem DB, headers no-store) lendo o `version.json` do dist servido. O frontend (`usePWA.ts`) compara as duas versões 8s após o load, a cada 90s e ao voltar ao primeiro plano; em divergência tenta o caminho normal do SW (update → onNeedRefresh) e, se em 5s não recarregou, desregistra o SW, apaga só os caches de precache (workbox/precache; preserva caches de dados runtime) e recarrega. Anti-loop via localStorage (`norte_version_guard`, máx 2 tentativas por versão-alvo). Resolve usuários presos com `sw.js` antigo em cache HTTP, cenário em que o `registerType: 'autoUpdate'` nunca dispara.
...
- **Unificação dos critérios de saturação (Maio/2026):** `is_curve_saturated` em `snapshot_service.py` (usado pelo cleanup script `scripts/cleanup_saturated_curves.py` e pelo `_pattern_is_saturated` no consumidor) passou a aplicar dois critérios em OR: (A) **forte/agressivo** — qualquer ponto em D-≥30 com pct≥0.95 (fisicamente impossível em curvas reais; mesma definição que `_resolve_hist_pattern` já usava internamente nos overrides — unifica para que cleanup e resolve concordem); (B) **legado/fraco** — ≥2 amostras com pct≥0.95 no quartil superior dos d_minus. Antes do fix, o helper canônico era só (B) e não detectava curvas como "Outono Vitória" travada em pct=1.0 em D-130, gerando inconsistência entre cleanup (não removia) e resolve (descartava no log mas o `_pattern_is_saturated` pós-resolve, que usava o helper fraco, deixava passar). Cleanup executado em PROD em 24/05/2026 removeu 15 curvas degeneradas (4 ano_referencia=2025, 11 ano_referencia=2026, total 3954 linhas), incluindo Inverno/Verão/Primavera Vitória 2026, Eco Run Palmas, Night Run Palmas/Porto Velho/São Luís/Teresina, S Run Rio Branco e outras.
...
- **Performance da Projeção de Inscritos (Julho/2026):** A invalidação do cache da Visão Consolidada deixou de apagar entradas e passou a marcá-las como stale + agendar recompute em background (single-flight com re-execução se uma mutação chegar durante o refresh) — saves de usuários simultâneos não forçam mais recálculo síncrono pesado na leitura seguinte. O caminho de cache-miss do `/consolidado` tem trava por chave (anti-estampede). Todo recompute (SWR, miss, pós-save) roda `congelar_cortes_para_eventos` antes do compute. `/pendencias` (por usuário) e `/cutoff-envio-map` ganharam cache em memória de 60s, limpo pela mesma invalidação central. Staleness visível: `/consolidado` sinaliza resposta stale via header `X-Consolidado-Stale`; o frontend exibe banner "Atualizando números..." e rebusca a cada 2,5s (máx. 8 tentativas) até chegar o valor fresco — se esgotar, o banner permanece (nunca esconder possível defasagem). Polling com sequenciamento de requisições (reqId) e guard pela aba ativa; sair da aba invalida respostas em voo e cancela o timer.
