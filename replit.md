...
- **Authentication:** Standard email/password login. Users can change their own password and upload a profile photo via `/perfil`. Profile photos are displayed in the header next to the user name. Backend routes: `POST/PUT/DELETE /api/profile/photo`, `PUT /api/profile/password`. Photos stored in `backend/uploads/profile_photos/`.
...
- **Marketing Performance (ISC Dashboard):** Displays Commercial Health Index (ISC) based on Acceleration Index, D-% Curve, and rolling 14-day sales average. Features detailed event pages with multiple tabs (Dashboard, Simulador, Precificação, Projeção, Complementares) including comparative curves and insights. ISC uses a `desvio+cap` model for calculation. Each event has a **Playbook** (A–I) based on stage (D-90→D-50 Analítico, D-50→D-32 Estratégico, D-32→D-0 Operacional) × ISC state (Forte/Estável/Fraco). Playbook is a structured object with name, objective, narrative, actions, KPIs, and cutoffs. Full playbook visualization available at `/marketing/playbook`. **Weekend Cutoff Anticipation:** When a strategic cutoff date (D-65, D-50, D-45, D-35, D-30, D-15, D-7) would fall on Saturday or Sunday, the system anticipates it to the previous Friday. Backend helper `_match_cutoff_with_weekend` checks if today (Brazil TZ) is Friday and matches `d_minus - 1` (Saturday) or `d_minus - 2` (Sunday) cutoffs. Frontend mirrors this logic using `America/Sao_Paulo` timezone. Anticipated cutoffs are flagged with `antecipado: true` in API responses and shown with an "antecipado" badge in the Nori alerts UI.
...
- **Cycling Scenarios (Cenários de Ciclismo):** When an event's modalidade is "Ciclismo", three sales scenarios are supported: "Inscrição Participação" (zero ticket), "Kit sem Bike" (lower ticket), and "Kit com Bike" (higher ticket). The Cadastro form (Atletas tab) shows 3 scenario cards instead of the single "Site" field, and the Faixas tab shows 3 columns (participação, sem_bike, com_bike). KitConfig admin has a `cenario_ciclismo` dropdown for cycling events. The ISC Dashboard EventDetail shows a "Cenários de Ciclismo" section with per-scenario volume, ticket, and revenue analysis. DB columns: `ciclismo_participacao_pago`, `ciclismo_sem_bike_pago`, `ciclismo_sem_bike_tkt_medio`, `ciclismo_com_bike_pago`, `ciclismo_com_bike_tkt_medio` on `cadastro_evento`; `cenario_ciclismo` on `kit_config`. Non-cycling events remain unchanged.
...
- **Cotação & Importação (Quote & Import):** Simplified FOB quote registration page. Three fields: Circuito (searchable dropdown with add-new), Produto (same), and Valor FOB (USD). Automatically fetches USD/BRL exchange rate via `/cotacoes/cambio` (economia.awesomeapi.com.br) and converts FOB to BRL at save time. Stores `taxa_cambio` and `valor_brl` alongside `valor_fob` in `cotacao_fob` table. Table displays FOB (USD), Câmbio, and Valor (BRL) columns. Backend: `/cotacoes/fob` CRUD + `/fob/circuitos` and `/fob/produtos` for distinct dropdown options. Old viagens/fornecedores/dashboard models preserved in DB but no longer used by the UI.
...
- **Projeção de Inscritos por Eventos e Áreas:** Allows users to input projected subscriber counts per event × predefined area (8 fixed categories: Atendimento, Marketing, Relações Institucionais, Company, Comercial, Cortesia RH, Relações Institucionais - Sem Kit, Company - Sem Kit). Admins configure which users can fill each area via `/projecao/areas/atribuir`. Full audit history in `projecao_inscritos_historico` tracks every creation/edit with campo_alterado, valor_anterior, valor_novo. Consolidated view sums real subscribers (from `vendas_diaria_snapshot` via SKU→grupo mapping) + all projections = total per event. Tables: `area_projecao`, `area_projecao_usuario`, `projecao_inscritos`, `projecao_inscritos_historico`. Frontend: `/projecao-inscritos` with 3 tabs (Projeções, Visão Consolidada, Configuração de Áreas). Backend: `/api/projecao/*` routes. Permission module: `projecao_inscritos`.
