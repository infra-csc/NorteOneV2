...
- **Commercial Actions Timeline:** Manages commercial actions (e.g., price changes, promotions) with CRUD operations, allowing tracking of their impact on sales before and after the action. Duplicate action prevention blocks creating the same action type within a 7-day window (backend returns HTTP 409, frontend pre-checks and shows alert). Active actions display pulsing badges in events table and "Ativa" badges with countdown on timeline items.
- **Sales Averages Analysis (Feb 2026):** `/marketing/eventos/{evento_id}/medias-vendas` endpoint returns daily sales averages with configurable periods (7, 14, 30, 60, 90 days). Uses real data from Ativo + Magento databases via SKU-based resolution through SkuMapping table. Frontend shows period selector, summary cards (media geral, 7d, 14d, 30d), and ComposedChart (bars for daily sales + line for 7-day moving average). Both "Vendas Diárias" and "Análise de Médias" charts use the same real data source (`fetch_daily_sales_ativo` with `dt_pedido` column, `fetch_daily_sales_magento`). The `generate_daily_sales_data()` simulated function is no longer used for event detail pages.
- **Year-over-Year Comparison Chart (Feb 2026):** Available exclusively in Event Detail page via `/marketing/curva-comparativa/{evento_id}` endpoint. **Aligned by "days before event"** instead of calendar months for honest cross-year comparison. Uses `data_evento` from `dim_projeto` matched via normalized name similarity (`_find_data_evento` helper). Each sale's `dias_antes_evento = data_evento - data_pedido` is calculated, then bucketed into 7-day (weekly) intervals. Data is returned as cumulative curve from D-max to D-0. Supports bar chart (semanal) and line chart (acumulado) views with toggles between Inscrições and Receita. Frontend shows event dates for both editions and subtitle "Alinhado por dias antes do evento". Daily sales queries (`_fetch_daily_sales_ativo_by_ids`, `_fetch_daily_sales_magento_by_ids`) GROUP BY DATE instead of MONTH. Falls back to monthly view (`_curva_comparativa_mensal_fallback`) when no `data_evento` match is found. Data consolidated from Ativo+Magento using `id_externo` from sku_mappings. Parallel query execution via ThreadPoolExecutor (4 workers), 5-minute per-event cache. **Sales attribution is by event edition year** (from sku_mappings.ano), not order date year — queries run separately for each edition's IDs, so curva totals match ISC Dashboard totals. General dashboard endpoint `/marketing/curva-comparativa` still available but not displayed in the main dashboard UI. **Projection line (Feb 2026):** For current year events, after the last bucket with real sales data, the backend calculates projected cumulative values using the rolling daily average (14-day or available period). Frontend renders the realized portion as a solid line and the projection as a dashed line (same color, reduced opacity) for clear visual distinction.
- **Simulated D- for Historical Years (Feb 2026):** `calculate_d_minus()` accepts optional `reference_year` parameter. When viewing a past year's event detail (e.g., ano=2025), D- is calculated as if today were transposed to that year (e.g., 12/Feb/2026 → 12/Feb/2025), simulating the perspective of that historical period. Events in past years are always marked as active (`is_active=True`).
