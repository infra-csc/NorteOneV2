...
- **Local PG Pool Isolation from Upstream Slowness:** Endpoints that mix local Postgres reads with Magento queries used to hold a local pool connection during the entire (potentially 60–90s) upstream call, eventually exhausting the local pool and breaking unrelated screens (e.g. `/api/admin/sku-mappings/grupos` returning 500). The fix lives in `backend/app/core/database.py` (`_active_local_sessions` thread-local + `release_local_db_connections()`) and is invoked at the top of `magento_run` in `backend/app/core/db_retry.py`. Before each Magento attempt the helper does a `session.rollback()` on every active local session **only when no writes are pending** (`not dirty/new/deleted`), returning the connection to the pool. Sessions stay usable — SQLAlchemy reacquires lazily on the next query (objects are expired, which is the standard SA 2.x behavior). Local PG pool also bumped to `pool_size=25, max_overflow=50` as safety net.
