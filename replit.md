...
- **Detecção de grupos vivos com fallback (UNION dim_projeto + cadastro_evento):** O `sincronizar_hoje_batch` (e também `snapshot_diario_batch`) em `backend/app/services/snapshot_service.py` agora identifica grupos live/hybrid usando UNION de **duas fontes**: `dim_projeto` E `cadastro_evento`. Antes, dependia somente de `dim_projeto.data_evento` — em produção essa tabela estava incompleta (faltava NRU26SP1, ex.), o que excluía silenciosamente Night Run SP 1ª Etapa do batch e fazia "vendas de hoje" aparecer como 0. Resolução do grupo a partir do CadastroEvento usa, em ordem: (a) `cadastro.sku`, (b) código do `dim_projeto` vinculado via `projeto_id`, (c) `id_evento_magento` casado com `SkuMapping`. DimProjeto é pré-carregado em uma query única para evitar N+1. Quando algum grupo é recuperado dessa fonte, é logado: `+N grupos live recuperados de cadastro_evento`.
...
- **Curva D-% override (lápis no card de Curva):** `PUT /api/admin/evento-grupos/{id}/curva-override` em `backend/app/api/routes/sku_mappings.py:set_curva_override` agora **invalida explicitamente o `EventoDetailSnapshot`** do grupo (`evento_id = grp_{nome}`) e o `curva_cache` do ano corrente, além do `_invalidate_all_marketing_caches()` (que só atinge caches em memória). Sem essa limpeza, o caminho snapshot-first do detalhe (`marketing.py:8711+`) servia o snapshot antigo com a curva anterior + recompute em background — o usuário via a curva voltar ao Linear logo após salvar e achava que não tinha persistido. No frontend, `handleSetOverride` em `EventDetail.tsx` agora aguarda (`await`) o `fetchEventRef.current(true)`, faz match tolerante (NFKC + trim + lowercase) ao buscar o `EventoGrupo` por nome, e exibe `alert` em caso de falha (antes silenciava no console).
- **`last_sync_hoje` atualizado em todos os pontos que rodam `sincronizar_hoje_batch`:** O carimbo "Inscrições às HH:MM" exibido no detalhe do evento (e usado pelo badge de freshness) é alimentado por `_last_sync_hoje_timestamp`. Antes, três caminhos rodavam o sync mas esqueciam de atualizar o carimbo: (1) consolidação diária das **04:00 BRT** (`_run_snapshot_consolidation` em `backend/app/core/cache.py`), (2) endpoint `POST /cache/sync-hoje` do botão "Sincronizar Hoje" na lista, e (3) endpoint `POST /eventos/{id}/atualizar-hoje` do botão "Atualizar Hoje" no detalhe. Adicionalmente, o **refresh diário das 05:00 BRT** (`_run_daily_refresh`) não chamava `sincronizar_hoje_batch` em momento nenhum — apenas warmup do ISC/lista/detalhe. Como o job das 04:00 frequentemente falha para grupos cujo Magento via SSH tunnel está instável, ao abrir o sistema de manhã o usuário via valores de ontem (ex.: 67 vendas) até o primeiro tick do agendador de 45min. Correções: (a) 04:00 chama `set_last_sync_hoje(time.time())` após o batch; (b) 05:00 ganhou um `sincronizar_hoje_batch` adicional como rede de segurança após o warmup completo, com `set_last_sync_hoje` em seguida; (c) os endpoints dos botões manuais agora chamam `set_last_sync_hoje` após sucesso (em `atualizar-hoje` somente quando `sync_failed=False`). Resultado: ao abrir de manhã o badge mostra "hoje às 05:XX" automaticamente, e os botões manuais refletem a hora do clique. Tudo sem criar nenhum botão novo na UI.
- **Margem realizada do "Resultado por Mês e Evento" alinhada ao Dash ISC:** A coluna "Margem Realizada" do dashboard inicial (`GET /dashboard/relatorio-financeiro` em `backend/app/api/routes/dashboard.py:get_relatorio_financeiro`) agora lê **`payload.evento.margemPorKit`** do `EventoDetailSnapshot` (mesma fonte do detalhe do evento) e usa `Σ(margemTotal por linha não-CONSOLIDADO)` quando há kit rows com `qtd > 0`, replicando 1:1 a fórmula do card "Margem Realizada" no Dash ISC (`EventDetail.tsx:1057`). Antes, o dashboard usava `margemRealizadaKitsTotal` do endpoint da lista, que computava `current_receita - Σ(custo × qty) - basico_avg × qty_remaining` com receita do `vendas_diaria_snapshot` e fallback de custo básico para vendas não mapeadas — divergindo do detalhe (que usa receita por bundle do Magento via `MargemBundleRevSnapshot` e ignora unmapped). Lookup é por `evento_id` (`grp_{nome}` para grupos, `str(projeto.id)` para standalones). Parsing isolado por evento (try/except interno) para que um payload malformado não desabilite override de todos os outros. Fallback graceful para `margemRealizadaKitsTotal` se snapshot não tiver `margemPorKit` ou tabela ainda não existir.
