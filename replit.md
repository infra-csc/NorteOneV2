...
- **Year-over-Year Comparison Chart (Feb 2026):** Available exclusively in Event Detail page via `/marketing/curva-comparativa/{evento_id}` endpoint. Shows monthly sales (inscriptions + revenue) for current year vs previous year. Supports bar chart (mensal) and line chart (acumulado) views with toggles between Inscrições and Receita. Data consolidated from Ativo+Magento using `id_externo` from sku_mappings (`id_evento` for Ativo, `location_pickup_id` for Magento) for correct database filtering. Uses date range filtering instead of YEAR() for better query performance. Parallel query execution via ThreadPoolExecutor (4 workers), 5-minute per-event cache. **Sales attribution is by event edition year** (from sku_mappings.ano), not order date year — queries run separately for each edition's IDs and group by month only, so curva totals match ISC Dashboard totals. General dashboard endpoint `/marketing/curva-comparativa` still available but not displayed in the main dashboard UI.
