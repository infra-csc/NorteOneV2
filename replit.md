...
- **Consolidated Events (EventoConsolidado):** Manual event grouping system that allows linking multiple SKU mappings (from ATIVO and MAGENTO sources) to a single consolidated event. Marketing Performance dashboards aggregate sales data by consolidated event while keeping years strictly separated (2025 data never mixes with 2026). Consolidated events use `ec_` prefix IDs (e.g., `ec_5`) to distinguish from standalone projects. Admin page at `/admin/eventos-consolidados` for CRUD and relationship management. Event detail view supports `?ano=YYYY` query parameter for year-specific data.
