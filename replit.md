...
- **Year-over-Year Comparison Chart (Feb 2026):** ISC Dashboard includes a comparison curve chart showing monthly sales (inscriptions + revenue) for current year vs previous year. Supports bar chart (mensal) and line chart (acumulado) views with toggles between Inscrições and Receita. Data aggregated from Ativo+Magento via `/marketing/curva-comparativa` endpoint with 5-minute cache and parallel query execution. Includes YTD summary cards with variation percentages. Also available in Event Detail page via `/marketing/curva-comparativa/{evento_id}` endpoint which filters by event-specific SKUs (supports both grouped and individual events).
