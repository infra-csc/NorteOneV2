...
- **Canal-based Inscription Filtering (Controle Diário):** `_fetch_daily_sales_ativo_by_ids` and `_fetch_today_sales_ativo_by_ids` use canal-based logic to classify inscriptions: cortesias are identified by `a.nr_preco = 0` (inscription price, not order total), and groups by `h.ds_categoria LIKE '%Grup%'`. The old `c.nr_total > 0` (order total) and `ds_categoria NOT LIKE '%ortesia%'` (category name) filters were removed to match the user's reference query exactly.
