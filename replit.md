...
- **User Activity Monitoring:** `dim_usuario.last_activity` column tracks when each user last made an authenticated request. Updated automatically in `get_current_user()` with 60s throttle (only writes to DB if last update was >60s ago). Admin page at `/admin/monitoramento` shows online status (green=online <5min, yellow=ausente <30min, gray=offline), summary cards, filterable user table with relative time display, and 30s auto-refresh. Protected by `admin_monitoramento` permission module. Backend endpoint: `GET /api/admin/user-activity` in `backend/app/api/routes/admin.py`.
...
- **Persistent Multi-Tier Cache:** `SmartCache` module uses dual-layer caching: in-memory (fast) + PostgreSQL `cache_entries` table (persistent across restarts, with `UNIQUE(cache_name, cache_key)` constraint). DB writes use atomic PostgreSQL UPSERT (`INSERT ... ON CONFLICT DO UPDATE`) via bounded `ThreadPoolExecutor(max_workers=2)`. Year-aware TTL: historical data cached permanently, current year data with 2-hour TTL but serves stale data while revalidating. DB reads preserve original `updated_at` timestamps for accurate TTL. `SmartCache.invalidate(cache_key)` removes from both in-memory dict AND PostgreSQL `cache_entries` table via `_delete_from_db()`, preventing stale DB fallback. On startup, all caches are loaded from PostgreSQL in <0.1s for instant data availability. `CacheRefreshScheduler` runs both periodic refresh (every 30 min) and daily full refresh at 07:00 BRT. Full warmup uses **3-step optimized pipeline**: Step 1 refreshes ISC data + batch pre-fetches ALL daily sales and category data from Ativo/Magento in 4 parallel queries; Step 2 processes all event tasks (detalhes, curvas, medias, insights) simultaneously using ThreadPoolExecutor(4 workers) with thread-ID scoped warmup cache (only warmup threads use pre-fetched data, live user requests always query fresh); Step 3 finalizes and clears warmup cache. SSH tunnel uses keepalive (30s interval) and 32KB buffer for improved throughput. Concurrent full-refresh prevention via atomic lock guard. Step 1 (ISC) is **resilient**: if Ativo DB fails, warmup continues with Magento data and reports partial warnings. Frontend uses stale-while-revalidate in-memory cache (30-min TTL, max 20 entries) with "Atualizar" (quick refresh) and "Atualizar Tudo" (full server-side refresh) buttons. Frontend `fullRefreshing` state syncs bidirectionally with server via polling (3s interval). Server-side last update timestamp displayed in dashboard. `POST /api/marketing/cache/refresh-all` triggers full warmup in background; `GET /api/marketing/cache/status` shows refresh progress (step/total, label, elapsed seconds, **sub_current/sub_total** for per-event granularity), timestamps, and `last_error` for warmup failures. Refresh button shows real-time **progress bar** with sub-progress ("Detalhes dos eventos (3/12)") and visual feedback on completion: green "Atualizado!" on success, red "Falha na atualização" on error, yellow "Tempo esgotado" on 3-min timeout. Partial warnings (e.g., Ativo timeout) shown as avisos instead of errors. Errors propagated to `ConnectionAlert` banner. Backend `_full_cache_warmup()` stores error messages via `set_last_refresh_error()`. MySQL connection pools use `pool_recycle=1800` (30 min), `connect_timeout=15`, `read_timeout=120` for SSH tunnel resilience. SQL queries use `MAX_EXECUTION_TIME(90000)` (90s) for Ativo/Magento queries. Magento rolling avg timeout set to 60s to accommodate variable query times (28-54s observed).
