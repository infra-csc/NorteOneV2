...
def seed_atletas(db: Session):
    if db.query(FatoAtletas).first():
        return
    ...
    for projeto in projetos:
        for categoria in categorias[:5]:
            qtd_orcado = random.randint(100, 500)
            qtd_projetado = int(qtd_orcado * random.uniform(0.9, 1.1))
            qtd_realizado = int(qtd_orcado * random.uniform(0.8, 1.2))
            
            db.add(FatoAtletas(
                projeto_id=projeto.id,
                categoria_atleta_id=categoria.id,
                tempo_id=tempo_dict.get(1),
                qtd_atletas_orcado=qtd_orcado,
                qtd_atletas_projetado=qtd_projetado,
                qtd_atletas_realizado=qtd_realizado,
                valor_inscricao_unitario=categoria.valor_inscricao_padrao,
                custo_kit_unitario_orcado=categoria.custo_kit_padrao,
                custo_kit_unitario_projetado=categoria.custo_kit_padrao,
                custo_kit_unitario_realizado=categoria.custo_kit_padrao,
                versao_projecao=1,
                created_by=1
            ))
