...
class _ModalidadeAliasCreate(BaseModel):
    raw_value: str
    canonical_value: str

class _ModalidadeAliasUpdate(BaseModel):
    raw_value: Optional[str] = None
    canonical_value: Optional[str] = None


@router.get("/modalidade-aliases")
def list_modalidade_aliases(
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(require_permission("admin_sku_mappings")),
):
    from app.models.dimensoes import ModalidadeAlias
    rows = db.query(ModalidadeAlias).order_by(ModalidadeAlias.raw_value).all()
    return [
        {
            "id": r.id,
...
@router.post("/modalidade-aliases", status_code=201)
def create_modalidade_alias(
    body: _ModalidadeAliasCreate,
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(require_permission("admin_sku_mappings")),
):
    from app.models.dimensoes import ModalidadeAlias
    from sqlalchemy.exc import IntegrityError as _IntErr
    alias = ModalidadeAlias(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=409, detail=f"Alias para '{body.raw_value}' já existe.")
    try:
        from app.services.detalhe_eventos_service import invalidate_alias_cache
        invalidate_alias_cache()
    except Exception:
        pass
    return {"id": alias.id, "raw_value": alias.raw_value, "canonical_value": alias.canonical_value}


@router.put("/modalidade-aliases/{alias_id}")
def update_modalidade_alias(
    alias_id: int,
    body: _ModalidadeAliasUpdate,
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(require_permission("admin_sku_mappings")),
):
    from app.models.dimensoes import ModalidadeAlias
    from sqlalchemy.exc import IntegrityError as _IntErr
    alias = db.query(ModalidadeAlias).filter(ModalidadeAlias.id == alias_id).first()
    if not alias:
        raise HTTPException(status_code=404, detail="Alias não encontrado.")
    if body.raw_value is not None:
        alias.raw_value = body.raw_value.strip()
    if body.canonical_value is not None:
        alias.canonical_value = body.canonical_value.strip()
    try:
        db.commit()
        db.refresh(alias)
    except _IntErr:
        db.rollback()
        raise HTTPException(status_code=409, detail=f"Já existe um alias com esse valor bruto.")
    try:
        from app.services.detalhe_eventos_service import invalidate_alias_cache
        invalidate_alias_cache()
    except Exception:
...
@router.delete("/modalidade-aliases/{alias_id}")
def delete_modalidade_alias(
    alias_id: int,
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(require_permission("admin_sku_mappings")),
):
