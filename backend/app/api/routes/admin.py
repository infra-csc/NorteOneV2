...
@router.get("/diag/event-min-prices")
def diag_event_min_prices(nome: str):
    from ...core.db_retry import magento_run, MagentoEngineUnavailable
    from sqlalchemy import text as _text
    def _work(conn):
        rows = conn.execute(_text("""
            SELECT cpe.entity_id AS bundle_id,
                   cpev_name.value AS nome_kit,
                   (SELECT COUNT(*) FROM catalog_product_entity_event_lot_price
                    WHERE entity_id=cpe.entity_id) AS total_lotes_bundle,
                   (SELECT MIN(pip.min_price) FROM catalog_product_index_price pip
                    WHERE pip.entity_id=cpe.entity_id AND pip.min_price>0) AS idx_min_price
            FROM catalog_product_entity cpe
            JOIN catalog_product_entity_varchar cpev_name
                   ON cpev_name.entity_id=cpe.entity_id AND cpev_name.attribute_id=73
            JOIN catalog_product_entity_varchar cpev_ev
                   ON cpev_ev.entity_id=cpe.entity_id AND cpev_ev.attribute_id=321
            JOIN catalog_product_entity_varchar cpev_ev_name
                   ON cpev_ev_name.entity_id=cpev_ev.value AND cpev_ev_name.attribute_id=73
            WHERE cpe.type_id='bundle' AND cpev_ev_name.value LIKE :nome
            ORDER BY cpev_name.value
        """), {"nome": f"%{nome}%"}).fetchall()
        cols = ["bundle_id","nome_kit","total_lotes_bundle","idx_min_price"]
        return [dict(zip(cols, r)) for r in rows]
    try:
        return magento_run(_work, label="diag-ev-mp", profile="request")
    except MagentoEngineUnavailable:
        return {"error": "Magento indisponível"}

@router.get("/diag/bundle-event-lots/{bundle_id}")
def diag_bundle_event_lots(bundle_id: int):
    from ...core.db_retry import magento_run, MagentoEngineUnavailable
    from sqlalchemy import text as _text
    def _work(conn):
        ev_id = conn.execute(_text("""
            SELECT value FROM catalog_product_entity_varchar
            WHERE entity_id=:bid AND attribute_id=321 LIMIT 1
        """), {"bid": bundle_id}).scalar()
        if not ev_id:
            return {"error": "bundle sem event_rel_pai"}
        lots = conn.execute(_text("""
            SELECT lot_id, lot_name, lot_value, lot_qty,
                   lot_sell_ends, lot_finished_date,
                   CASE WHEN lot_sell_ends IS NULL OR lot_sell_ends >= NOW()
                        THEN 'ativo' ELSE 'expirado' END AS status
            FROM catalog_product_entity_event_lot_price
            WHERE entity_id = :eid
            ORDER BY record_id DESC
        """), {"eid": ev_id}).fetchall()
        cols = ["lot_id","lot_name","lot_value","lot_qty","lot_sell_ends","lot_finished_date","status"]
        return {"bundle_id": bundle_id, "event_entity_id": ev_id,
                "lots": [dict(zip(cols, r)) for r in lots]}
    try:
        return magento_run(_work, label="diag-ev-lots", profile="request")
    except MagentoEngineUnavailable:
        return {"error": "Magento indisponível"}

@router.get("/diag/bundle-selection-prices/{bundle_id}")
def diag_bundle_sel_prices(bundle_id: int):
    from ...core.db_retry import magento_run, MagentoEngineUnavailable
    from sqlalchemy import text as _text
    def _work(conn):
        # bundle selections + their prices
        sels = conn.execute(_text("""
            SELECT cps.selection_id, cps.product_id, cps.selection_price_type,
                   cps.selection_price_value, cps.selection_qty,
                   cpev.value AS child_name,
                   cpep.value AS child_eav_price
            FROM catalog_product_bundle_selection cps
            JOIN catalog_product_bundle_option cpo ON cpo.option_id=cps.option_id
            LEFT JOIN catalog_product_entity_varchar cpev
                   ON cpev.entity_id=cps.product_id AND cpev.attribute_id=73
            LEFT JOIN catalog_product_entity_decimal cpep
                   ON cpep.entity_id=cps.product_id
                  AND cpep.attribute_id=(SELECT attribute_id FROM eav_attribute
                                         WHERE attribute_code='price' AND entity_type_id=4 LIMIT 1)
            WHERE cpo.parent_id=:bid
            ORDER BY cps.selection_id
        """), {"bid": bundle_id}).fetchall()
        # index prices for this bundle (just group 0 and 32000)
        idx = conn.execute(_text("""
            SELECT customer_group_id, price, final_price, min_price, max_price
            FROM catalog_product_index_price
            WHERE entity_id=:bid AND customer_group_id IN (0,32000)
            ORDER BY customer_group_id
        """), {"bid": bundle_id}).fetchall()
        return {
            "bundle_id": bundle_id,
            "selections": [{"sel_id":r[0],"prod_id":r[1],"price_type":r[2],
                            "price_value":float(r[3]) if r[3] else None,
                            "qty":float(r[4]) if r[4] else None,
                            "child_name":r[5],
                            "child_eav_price":float(r[6]) if r[6] else None} for r in sels],
            "index_prices": [{"grp":r[0],"price":float(r[1]) if r[1] else None,
                              "final_price":float(r[2]) if r[2] else None,
                              "min_price":float(r[3]) if r[3] else None,
                              "max_price":float(r[4]) if r[4] else None} for r in idx],
        }
    try:
        return magento_run(_work, label="diag-sel", profile="request")
    except MagentoEngineUnavailable:
        return {"error": "Magento indisponível"}
