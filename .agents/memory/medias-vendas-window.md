...
**Separate from the ISC card:** the `/medias-vendas` endpoint (`get_sales_averages`) does NOT feed the ISC card. The ISC card's rolling/IA medias for live/future events come independently from `get_isc_totals_from_snapshot` (which already windows to yesterday). So a change to `get_sales_averages` cannot move the ISC value — if ISC dev-vs-prod diverge, look at snapshot data freshness or stale frontend/browser cache, not the medias-vendas code. Confirmed by reproducing every ISC path (cache-media, daily-dict, normalized) against the live DB and getting the same value.
